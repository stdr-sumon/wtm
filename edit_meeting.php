<?php
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = pg_query($conn, "select * from edit_info_meeting('$id');");
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        $sql = pg_query($conn, "select * from insert_update_meeting('$place', '$start_time', '$end_time', '$is_active', 'update', '$id');");
                  ...
                  <form method="post">
                        <div class="row">
                            <div class="input-field col s12">
                                <input placeholder="Location" id="place" name="place" type="text" class="validate" value="<?php echo $row['place']; ?>">
                        ...
                        <div class="row">
                            <div class="input-field col s6">
                                <?php
                                    $start_date = date("c", strtotime($row['start_date']));
                                    list($Date)=explode('+', $start_date);
                                    $start_date = substr($Date,0,-3);

                                    $end_date = date("c", strtotime($row['end_date']));
                                    list($Datee)=explode('+', $end_date);
                                    $end_date = substr($Datee,0,-3);
                        ...
                        <div class='row'>
                            ...
                            <select name="visibility" required>
                                <option <?php echo ($row['is_active'] == 'f') ? 'selected' : '';?> value="f">Hide</option>
                                <option <?php echo ($row['is_active'] == 't') ? 'selected' : '';?> value="t">Active</option>
                            </select>
